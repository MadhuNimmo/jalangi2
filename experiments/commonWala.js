...
function main() {
        ...
        if (SCGextension == "txt") {
                ...
                /*var cnt=0
                for(key in statCallGraph){
                        for(val in statCallGraph[key]){
                                cnt+=1
                        }
                }*/
                //console.log(cnt)
                var statCallGraphEdit = jsonToGraph(statCallGraph);
        }
...
function common(diffs, items) {
        ...
        var output = new Graph();//clonedeep(diffs)
        diff_keys = [...diffs.getKeys()]
        item_keys = [...items.getKeys()]
        console.log(diff_keys.length)
        for (var diff_key of diff_keys) {
                reg_key=regex.exec(diff_key)
                regex.lastIndex = 0;
                
                diffkey_pattern =  reg_key == null? null:reg_key.slice(1, 3).every(element => element != undefined) ? reg_key.slice(1, 3):null
                var values = diffs.getValues(diff_key);
                /*if (diffkey_pattern == null){
                        natcnt+=1
                }
                if ((diff_key).includes("app.js") || (diff_key).includes("base.js")){
                        appcnt+=1;
                }
                else if ((diff_key).includes("knockout-latest.js") || (diff_key).includes("director.js")){
                        frmcnt+=1;
                }*/
                for (value of values) {
                                ...
                                for (key of items.getKeys()) {
                                        ...
                                        if (diffkey_pattern.every(pattern => key.toString().includes(pattern))) {
                                                ...
                                                for (val of items.getValues(key)) {
                                                        //console.log(diffkey_pattern,key,val);
                                                        output.addEdge(key, val)

                                                        if (diffval_pattern!==null && diffval_pattern.every(pattern => val.toString().includes(pattern))){
        ...
        console.log(cnt,appcnt,frmcnt,natcnt)
        return output;
}
