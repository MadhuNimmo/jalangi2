(function () {
        ...
        J$.analysis = {
                ...
                invokeFunPre: function (iid, f, base, args, isConstructor, isMethod, functionIid, functionSid) {
                        console.log(getLoc(J$.getGlobalIID(iid)),f.name,getLoc(J$.getGlobalIID(functionIid)))
                        var funName = f.name;
                        var giid = J$.getGlobalIID(iid);
                ...
                functionEnter: function (iid, f, dis, args) {
                        console.log(getLoc(J$.getGlobalIID(iid)),f.name)
                        var funName = f.name;
                        var giid = J$.getGlobalIID(iid);
                        iidToFunName[giid] = funName == "" ? "anon" : funName;
                        ...
                        if (iidToCallerLoc[giid] == undefined) {
                                ...
                                if (f.name.startsWith("set ") || f.name.startsWith("get ")){
                                        ...
                                        if ((["apply","call"].includes(callerName) || callerName.startsWith("bound ")) && applyStack.length>0){  
                                                ...
                                                if(callerName==="anon"){
                                                        console.log(callStack.length,getLoc(callStack[callStack.length-1]),getLoc(giid))
                                                }
...
/*
node src/js/commands/jalangi.js --inlineIID --inlineSource --analysis experiments/metrics/DynNative.js experiments/example.js
node src/js/commands/instrument.js --inlineIID --inlineSource -i --inlineJalangi --analysis src/js/sample_analyses/ChainedAnalyses.js --analysis src/js/sample_analyses/dlint/Utils.js --analysis experiments/metrics/DynNative.js --outputDir /tmp experiments/html/
