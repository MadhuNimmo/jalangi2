...
async function readLargeJSONFile(SCGFilename) {
	return new Promise((resolve, reject) => {
		// const SCG_ = [];

		const stream = fs.createReadStream(SCGFilename, {
			encoding: "utf8",
		});
		...
		parser.on("data", async function (data) {
			// Process each chunk of parsed data here
			SCG.push(data);
		});
...
async function main() {
	...
	if (metricType == "Metric1" || metricType == "EdgeDiff") {
		...
		statCallGraphEdit = await readLargeJSONFile(SCGFilename)
			.then((SCG1) => {
				SCG = SCG1.reduce((acc, obj, i) => {
					acc[i] = obj;
					return acc;
				}, {});
				...
				return jsonToGraph(statCallGraph);
			})
			.catch((err) => {
				console.error("Error while parsing JSON:", err);
			});

		// console.log(statCallGraphEdit);
...
async function processInputs(DCG, SCG, JSfiles, metric) {
