        ...
        if(typeof item["trace"][0] === "string"){
                ...
                if (Array.isArray(trace) && trace.length>0){
                        ...
                        if(Array.isArray(trace[0])){
                                if(trace[0][0]==="With involved"){
                                        missingPaths.push("With involved")
...
function findPath(src,sink,inter,applycall = false){
        ...
        if(stat_src==-1){ return "Function Constructor Involved: "+src_ptrn+" and "+ (stat_sink === null || stat_sink.length === 0?sink_ptrn:stat_sink) }
        if(stat_src==-5){ return "Calls to bounded functions"}
        if(stat_src==-2){ 
                return("Eval Involved: "+src_ptrn+" and "+sink_ptrn)
        }else if (stat_src==-3){
